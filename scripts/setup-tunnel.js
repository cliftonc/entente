...
function installCloudflared() {
  ...
  try {
    if (process.platform === 'darwin') {
      ...
      error(
        'Please install cloudflared manually from https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/'
      )
      process.exit(1)
    }
  } catch (e) {
...
async function main() {
  ...
  try {
    ...
    execSync(`cloudflared tunnel route dns ${tunnelId} ${TUNNEL_NAME}.${DOMAIN}`, {
      stdio: 'inherit',
    })
