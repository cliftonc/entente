...
program
  ...
  .option(
    "--spec <file>",
    "Path to OpenAPI spec file to upload (providers only)",
  )
  .option(
    "-v, --spec-version <version>",
    "Spec version (required if --spec provided)",
  )
  .option(
  ...
  .action(async (options) => {
    try {
      ...
      console.log(
        chalk.blue("📦"),
        `Registering ${options.type} service ${options.name}...`,
      );
      ...
      if (options.spec) {
        if (options.type !== "provider") {
          throw new Error(
            "OpenAPI specs can only be uploaded for provider services",
          );
        }
...
program
  ...
  .description(
    "Register a provider with package.json and optionally upload OpenAPI spec",
  )
...
program
  ...
  .option(
    "-c, --consumer <consumer>",
    "Legacy: Consumer service name (use --service instead)",
  )
  .option(
  ...
  .action(async (options) => {
    try {
      ...
      if (!service || !version) {
        try {
          ...
          if (service && version) {
            console.log(
              chalk.gray(
                `Using service "${service}" version "${version}" from ${packageJsonPath}`,
              ),
            );
          }
        } catch (error) {
      ...
      if (!service || !version) {
        console.error(
          chalk.red("Error: Service name and version are required"),
        );
        process.exit(1);
      }
      ...
      if (result.canDeploy) {
        ...
        if (result.compatibleServices && result.compatibleServices.length > 0) {
          ...
          for (const compatibleService of result.compatibleServices) {
            const status = compatibleService.verified
              ? chalk.green("✅")
              : chalk.yellow("⚠️");
            const typeLabel = compatibleService.type
              ? `[${compatibleService.type}]`
              : "";
            const deployedLabel =
              compatibleService.activelyDeployed === false
                ? " (not deployed)"
                : "";
            console.log(
