...
program
  ...
  .description('Register a service (consumer or provider) with package.json and optionally upload OpenAPI spec')
  ...
  .option('-e, --environment <environment>', 'Environment for spec (required if --spec provided)', 'development')
  ...
  .action(async (options) => {
    try {
      ...
      if (options.spec) {
        ...
        console.log(chalk.blue('📤'), `Uploading OpenAPI spec for ${options.name}@${options.specVersion}...`)
        await uploadSpec({
...
program
  ...
  .requiredOption('-t, --type <type>', 'Service type: consumer or provider')
  .option('-D, --dependencies <deps>', 'Dependencies in format provider:version,provider2:version2 (consumers only)')
    ...
    try {
      ...
      if (options.type === 'consumer') {
        if (!options.dependencies) {
          throw new Error('Dependencies are required for consumer deployments')
        }

        // Parse dependencies string
        const dependencies = options.dependencies.split(',').map((dep: string) => {
          const [provider, version] = dep.trim().split(':')
          if (!provider || !version) {
            throw new Error(`Invalid dependency format: ${dep}. Use provider:version`)
          }
          return { provider, version }
        })

        console.log(chalk.blue('🚀'), `Deploying consumer ${options.name}@${options.serviceVersion} to ${options.environment} with ${dependencies.length} dependencies...`)
        ...
        if (options.dependencies) {
          console.log(chalk.yellow('⚠️'), 'Dependencies option ignored for provider deployments')
        }

        console.log(chalk.blue('🚀'), `Deploying provider ${options.name}@${options.serviceVersion} to ${options.environment}...`)
...
program
  .command('upload-spec')
  .description('Upload OpenAPI specification to central service (auto-registers provider if needed)')
  ...
  .action(async (options) => {
    try {
      console.log(chalk.blue('📤'), `Uploading OpenAPI spec for ${options.service}@${options.serviceVersion} to ${options.environment}...`)
      await uploadSpec({
...
program
  ...
  .action(async (options) => {
    try {
      console.log(chalk.blue('🚀'), `Recording deployment of ${options.service}@${options.serviceVersion} to ${options.environment}...`)
...
program
  ...
  .option('-s, --service <service>', 'Service name (consumer or provider) - defaults to package.json name')
  ...
  .option('--consumer-version <version>', 'Legacy: Consumer version (use --service-version instead)')
  .action(async (options) => {
    try {
      ...
      if (!service || !version) {
        try {
          ...
          if (service && version) {
            console.log(chalk.gray(`Using service "${service}" version "${version}" from ${packageJsonPath}`));
          }
        } catch (error) {
          console.error(chalk.red('Error reading package.json:'), error instanceof Error ? error.message : 'Unknown error');
          if (!service || !version) {
            console.error(chalk.red('Service name and version are required. Provide them via --service and --service-version or ensure package.json exists.'));
            process.exit(1);
          }
        }
      }
      ...
      console.log(chalk.blue('🔍'), `Checking if ${service}@${version} can deploy to ${options.environment}...`)
      ...
      if (result.canDeploy) {
        console.log(chalk.green('✅'), `${service} v${version} can deploy to ${options.environment}`)
        console.log('')
        ...
        if (result.compatibleServices && result.compatibleServices.length > 0) {
          ...
          for (const compatibleService of result.compatibleServices) {
            ...
            const deployedLabel = compatibleService.activelyDeployed === false ? ' (not deployed)' : ''
            console.log(`  - ${compatibleService.service} v${compatibleService.version} ${status} (${compatibleService.interactionCount} interactions) ${typeLabel}${deployedLabel}`)
          }
        }
        ...
        console.log(chalk.red('❌'), `${service} v${version} cannot deploy to ${options.environment}`)
        console.log('')
