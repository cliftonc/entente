...
export const extractOperationFromSpec = (
  ...
  return matchingOperation?.iid || matchingOperation?.operationId || extractOperationFromPath(method, path);
};
...
export const extractOperationFromPath = (
  ...
  if (cleanPath) {
    ...
    const singularPath = cleanPath.endsWith('s') && cleanPath.length > 1
      ? cleanPath.slice(0, -1)
      : cleanPath;
