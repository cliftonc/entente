...
import { interactions, services, verificationTasks } from '../../db/schema'
...
async function createVerificationTaskFromInteraction(
  ...
  if (!providerId || !consumerId) {
    console.log(`ℹ️  Skipping verification task creation: missing provider (${providerId}) or consumer (${consumerId}) ID`)
    return
  }
  ...
  if (existingTask) {
    ...
    console.log(`✅ Updated verification task with ${allInteractions.length} interactions for ${consumerName}@${consumerVersion} -> ${providerName}`)
  } else {
    ...
    try {
      ...
      console.log(`✅ Created verification task with ${allInteractions.length} interactions for ${consumerName}@${consumerVersion} -> ${providerName}`)
    } catch (error: any) {
      // Handle unique constraint violation (race condition)
      if (error.code === '23505') {
        console.log(`📋 Race condition detected for verification task: ${consumerName}@${consumerVersion} -> ${providerName}`)
      } else {
...
interactionsRouter.get('/', async (c) => {
  ...
  console.log(`📋 Retrieved ${clientInteractions.length} interactions with filters: provider=${provider || 'all'}, consumer=${consumer || 'all'}`)
...
interactionsRouter.post('/', async (c) => {
  ...
  const existingInteraction = await db.query.interactions.findFirst({
    where: and(
      eq(interactions.tenantId, tenantId),
      eq(interactions.hash, hash)
    ),
  })

  if (existingInteraction) {
    console.log(`📋 Duplicate interaction found: ${interaction.consumer} -> ${interaction.service}.${interaction.operation}`)
  ...
  try {
    ...
    console.log(`📝 Recorded interaction: ${interaction.consumer}${consumer?.id ? ` (${consumer.id})` : ''} -> ${interaction.service}${provider?.id ? ` (${provider.id})` : ''}.${interaction.operation}`)

    return c.json({ status: 'recorded', id: newInteraction.id }, 201)
  } catch (error: any) {
    // Handle unique constraint violation (race condition)
    if (error.code === '23505' && error.constraint?.includes('tenant_hash_unique')) {
      console.log(`📋 Race condition detected for interaction: ${interaction.consumer} -> ${interaction.service}.${interaction.operation}`)
...
interactionsRouter.get('/consumer/:consumer', async (c) => {
  ...
  const whereConditions = [
    eq(interactions.tenantId, tenantId),
    eq(interactions.consumer, consumer),
  ]
...
interactionsRouter.post('/batch', async (c) => {
  ...
  console.log(`📋 Batch processed ${incomingInteractions.length} interactions: ${results.recorded} recorded, ${results.duplicates} duplicates, ${results.errors} errors`)
