...
export const verificationTasks = pgTable('verification_tasks', {
  ...
  consumerId: uuid('consumer_id').references(() => services.id).notNull(),
  ...
  tenantConsumerProviderVersionUnique: unique().on(table.tenantId, table.consumerId, table.consumerVersion, table.providerId)
}))
